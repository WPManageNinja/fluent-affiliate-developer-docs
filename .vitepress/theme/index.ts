      ...
      window.setTimeout(() => {
        ...
        wrapper.innerHTML = `
          <details data-playground-instructions>
            ...
            <div>
              <p>Test FluentAffiliate endpoints against your own WordPress site. Requests are sent directly from your browser.</p>
              <ol>
          ...
          <div class="playground-server-box">
            <label for="fa-server-url">Server URL</label>
            <input id="fa-server-url" data-server-url-input type="text" placeholder="your-site.test" />
            <p data-server-url-display>https://your-site.test/wp-json/fluent-affiliate/v2</p>
          </div>
        `
        ...
        const savedDomain = window.localStorage.getItem('fluentaffiliate_api_server_url')
        const savedAuth = window.localStorage.getItem('fluentaffiliate_api_auth_credentials')
        ...
        const updateServerUrl = () => {
          ...
          const serverUrl = `https://${host}/wp-json/fluent-affiliate/v2`
          display.textContent = serverUrl
          window.__customServerUrl = serverUrl

          if (host && host !== 'your-site.test') {
            window.localStorage.setItem('fluentaffiliate_api_server_url', host)
          }
        }
        ...
        authInput?.addEventListener('input', () => {
          ...
          if (value) {
            window.localStorage.setItem('fluentaffiliate_api_auth_credentials', value)
          } else {
            window.localStorage.removeItem('fluentaffiliate_api_auth_credentials')
          }
        })
    ...
    window.fetch = function (...args: Parameters<typeof fetch>) {
      if (window.__customServerUrl && typeof args[0] === 'string') {
        args[0] = args[0].replace(
          /^https?:\/\/[^/]+\/wp-json\/fluent-affiliate\/v2/,
